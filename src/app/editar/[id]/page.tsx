...
const EditarClientePage = () => {
  ...
  const fetchCliente = async () => {
    try {
      const response = await axios.get(`http://localhost:8080/clientes/${id}`); // Requisição para buscar o cliente específico
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      await axios.put(`http://localhost:8080/clientes/${id}`, cliente); // Atualiza o cliente na API
